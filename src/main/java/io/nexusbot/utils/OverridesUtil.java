            ...
            if (type.equals("role")) {
                ...
                if (role != null) {
                    voiceChannel.upsertPermissionOverride(role)
                            .setAllowed(allow)
                            .setDenied(deny)
                            .queue(_ -> {}, error -> LOGGER.info("Error while upserting role permissions: {}", error.getMessage()));
                }
                ...
                if (member != null) {
                    voiceChannel.upsertPermissionOverride(member)
                            .setAllowed(allow)
                            .setDenied(deny)
                            .queue(_ -> {}, error -> LOGGER.info("Error while upserting member permissions: {}", error.getMessage()));
                }
            }
        }
    }
